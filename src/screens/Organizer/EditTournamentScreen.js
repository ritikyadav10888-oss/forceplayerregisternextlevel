...
import { View, Text, StyleSheet, ScrollView, TextInput, TouchableOpacity, Platform, KeyboardAvoidingView, Alert, ActivityIndicator, Image } from 'react-native';
...
import { getTournamentById, updateTournament, deleteTournament } from '../../services/tournamentService';
...
export default function EditTournamentScreen({ navigation, route }) {
        ...
        try {
            ...
            if (data) {
                setFormData({
                    ...
                    scheduleType: data.scheduleType || 'Single Elimination',
                    scoringMetric: data.scoringMetric || 'Points'
                });
            }
        } catch (error) {
            console.error(error);
    ...
    const handleDelete = async () => {
        Alert.alert(
            "Delete Tournament",
            "Are you sure you want to delete this tournament? This action cannot be undone and all registrations will be lost.",
            [
...
const styles = StyleSheet.create({
    ...
    scrollContent: { flexGrow: 1, padding: spacing.l, paddingBottom: 100, paddingTop: 0 },
    ...
    deleteButton: { flex: 0.4, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', borderRadius: 16, borderWeight: 1, borderColor: colors.error, backgroundColor: '#FEF2F2', paddingVertical: 18, borderWidth: 1 },
    deleteButtonText: { color: colors.error, fontSize: 16, fontWeight: '800', marginLeft: 8 },
    createButton: { flex: 1, borderRadius: 20, overflow: 'hidden', shadowColor: colors.primary, shadowOpacity: 0.3, shadowRadius: 10, elevation: 6 },
